...
public class PiggyAdminCommand {
        ...
        private static int checkCheats(CommandContext<CommandSourceStack> context) {
                ...
                context.getSource().sendSuccess(
                                () -> Component.literal(
                                                "Piggy Cheats are currently: " + (allowed ? "ALLOWED" : "FORBIDDEN")),
                                false);
                return 1;
        }
        ...
        private static int listFeatures(CommandContext<CommandSourceStack> context) {
                ...
                context.getSource().sendSuccess(
                                () -> Component.literal("=== Piggy Features ===").withStyle(ChatFormatting.GOLD),
                                false);

                context.getSource().sendSuccess(
                                () -> Component.literal(
                                                "Master Switch: " + (config.allowCheats ? "ALLOWED" : "FORBIDDEN"))
                                                .withStyle(config.allowCheats ? ChatFormatting.GREEN
                                                                : ChatFormatting.RED),
                                false);
                ...
                for (CheatFeature feature : CheatFeatureRegistry.getAllFeatures()) {
                        ...
                        context.getSource().sendSuccess(
                                        () -> Component.literal("• " + feature.displayName() + " (" + feature.id()
                                                        + "): " + status)
                                                        .withStyle(color),
                                        false);

                        context.getSource().sendSuccess(
                                        () -> Component.literal("  " + feature.description())
        ...
        private static int checkFeature(CommandContext<CommandSourceStack> context) {
                ...
                context.getSource().sendSuccess(
                                () -> Component.literal(feature.displayName() + " is currently: "
        ...
        private static int setFeature(CommandContext<CommandSourceStack> context, boolean enable) {
                ...
                context.getSource().sendSuccess(
                                () -> Component.literal(
                                                feature.displayName() + " is now: " + (enable ? "ENABLED" : "DISABLED"))
